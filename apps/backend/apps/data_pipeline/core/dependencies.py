...
class TableDependencyResolver:
    ...
    def add_shared_field_dependency(
        self, source_table: str, target_tables: list[str], field_name: str, cleaning_rules: list[str] | None = None
    ) -> None:
        ...
        for target_table in target_tables:
            dependency = TableDependency(
                target_table=target_table, source_table=source_table, shared_fields=[field_name]
            )
            self.dependencies[target_table].append(dependency)

        self.logger.info(f"Added shared field dependency: {source_table}.{field_name} -> {target_tables}")
    ...
    def get_shared_field_mapping(self, source_table: str, field_name: str) -> SharedFieldMapping | None:
    ...
    def is_header_table(self, table_name: str) -> bool:
        ...
        return any(mapping.source_table == table_name for mapping in self.shared_fields.values())
...
class CrossTableCleaningCache:
    ...
    def cache_cleaned_field_values(self, table_name: str, field_name: str, mappings: dict[str, str]) -> None:
        ...
        if len(mappings) > self.max_cache_size:
            self.logger.warning(f"Cache size limit exceeded for {cache_key}: {len(mappings)} > {self.max_cache_size}")
        ...
        self.logger.info(f"Cached {len(mappings)} cleaned values for {cache_key}")

    def get_cleaned_value(self, table_name: str, field_name: str, original_value: str) -> str | None:
    ...
    def clear_cache(self, table_name: str | None = None, field_name: str | None = None) -> None:
...
def setup_academic_dependencies(resolver: TableDependencyResolver) -> None:
    ...
    resolver.register_table_metadata(
        "academicclasses",
        {
            "estimated_rows": 500,
            "filters": {"is_shadow": 0},
