"""Create canonical requirements for Business Administration major from BA-BUSADMIN.csv.

This migration script creates exactly 44 canonical requirements for the Business
Administration major, validating that all courses exist as active courses and
that the total credits equals 133 (including COMP-210 at 4 credits).
"""
...
class Command(BaseMigrationCommand):
    ...
    help = "Create canonical requirements for Business Administration major from BA-BUSADMIN.csv"
    ...
    def execute_migration(self, *args, **options):
        ...
        # Expected total credits for BA Business Administration
        EXPECTED_TOTAL_CREDITS = 133

        # Load BA-BUSADMIN.csv
        csv_path = "data/legacy/BA-BUSADMIN.csv"
        ...
        # Get Business Administration major
        try:
            major = Major.objects.get(code="BUSADMIN")
            self.record_success("major_found", 1)
            self.stdout.write(f"Business Administration major found: {major.name}")
        except Major.DoesNotExist:
            self.record_rejection("major_not_found", "BUSADMIN", "Business Administration major not found")
            return
        ...
        if total_credits != EXPECTED_TOTAL_CREDITS:
            self.record_rejection(
                "credit_mismatch",
                f"Total: {total_credits}",
                f"Expected {EXPECTED_TOTAL_CREDITS} credits, got {total_credits}",
            )
        else:
        ...
        # Special validation for COMP-210
