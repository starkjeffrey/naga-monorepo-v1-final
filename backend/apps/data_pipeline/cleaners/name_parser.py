"""Student name parsing utilities for legacy data transformation.
...
4. Post-processing indicators:
   ...
   - <sponsor_name> = SPONSORED student with sponsor name
   - {AF} = subject to admin fees
   - (ST) = staff scholarship
...
class StudentNameData:
    ...
    has_staff_scholarship: bool = False
    """True if student has staff scholarship (ST) marker."""
    ...
    def has_special_status(self) -> bool:
        ...
        return (self.is_sponsored or self.is_frozen or
                self.has_admin_fees or self.has_staff_scholarship)
    ...
    def status_summary(self) -> str:
        ...
        if self.has_staff_scholarship:
            statuses.append("Staff Scholarship")
...
class StudentNameParser:
    ...
    def __init__(self):
        ...
        # Also allow parentheses but only if they contain known markers
        self._name_pattern = re.compile(r"^[A-Za-z\s\-]+")
        # Pattern for sponsor indicators <something>
        self._sponsor_pattern = re.compile(r"<([^>]*)>")
        # Pattern for staff scholarship (ST) - must be in parentheses
        self._staff_scholarship_pattern = re.compile(r"\(ST\)", re.IGNORECASE)
    ...
    def parse(self, raw_name: str) -> StudentNameData:
        ...
        try:
            # Pre-process: Check for (ST) marker and remove it early
            has_st_marker = bool(self._staff_scholarship_pattern.search(raw_name))
            # Remove (ST) from the string before processing
            if has_st_marker:
                raw_name = self._staff_scholarship_pattern.sub("", raw_name).strip()
            ...
            if not first_alpha_match:
                return StudentNameData(
                    clean_name="",
                    has_staff_scholarship=has_st_marker,
            ...
            result = StudentNameData(
                clean_name=clean_name,
                raw_indicators=indicators_string,
                has_staff_scholarship=has_st_marker  # Set from pre-processing
            )
            ...
            return StudentNameData(
                clean_name="",
                has_staff_scholarship=has_st_marker if 'has_st_marker' in locals() else False,
                parsing_warnings=[f"Unexpected parsing error: {str(e)}"]
            )
    ...
    def _parse_status_indicators(self, indicators: str, result: StudentNameData) -> None:
        ...
        if "{AF}" in indicators.upper():
            result.has_admin_fees = True

        # Also check for (ST) in the indicators string in case it appears after the name
        if "(ST)" in indicators.upper():
            result.has_staff_scholarship = True
...
def parse_student_name(raw_name: str) -> StudentNameData:
    ...
    Example:
        ...
        >>> result = parse_student_name("BO (ST) MESA")
        >>> result.clean_name
        'BO MESA'
        >>> result.has_staff_scholarship
        True
    """
...
def has_special_status(raw_name: str) -> bool:
    ...
    Returns:
        True if name contains sponsored, frozen, admin fee, or staff scholarship indicators
    """
    result = parse_student_name(raw_name)
