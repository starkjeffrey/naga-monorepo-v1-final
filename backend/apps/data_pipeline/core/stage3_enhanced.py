...
from .dependencies import TableDependencyResolver, CrossTableCleaningCache
...
class DependencyAwareStage3Clean:
    ...
    def execute_multi_table_pipeline(self, 
        ...
        try:
            ...
            self.logger.info(
                f"Dependency-aware Stage 3 completed in {execution_time:.2f}s - "
                f"{total_processed} total rows processed"
            )
    ...
    def _process_detail_table(self, config: TableConfig, dry_run: bool) -> Dict[str, Any]:
        ...
        cleaner = CachedFieldStage3Cleaner(
            config, self.logger, self.run_id, 
            self.dependency_resolver, self.cleaning_cache
        )
    ...
    def _cache_shared_fields(self, config: TableConfig, result: Dict[str, Any]) -> None:
        ...
        dependent_tables = self.dependency_resolver.get_dependent_tables(config.table_name)
        ...
        with connection.cursor() as cursor:
            ...
            for mapping in config.column_mappings:
                ...
                if shared_field_info:
                    self._extract_and_cache_field_mappings(
                        cursor, config, mapping, shared_field_info
                    )
                    
    def _extract_and_cache_field_mappings(self, 
                                         cursor,
                                         config: TableConfig, 
                                         mapping,
                                         shared_field_info) -> None:
        ...
        # Cache the mappings
        self.cleaning_cache.cache_cleaned_field_values(
            config.table_name, mapping.source_name, mappings
        )
        
        self.logger.info(
            f"Cached {len(mappings)} mappings for {config.table_name}.{mapping.source_name}"
        )
        ...
        for dependency in dependencies:
            for shared_field in dependency.shared_fields:
                if column_name and shared_field in column_name:
                    cached_value = self.cleaning_cache.get_cleaned_value(
                        dependency.source_table, shared_field, value
                    )
                    if cached_value is not None:
                        return cached_value
