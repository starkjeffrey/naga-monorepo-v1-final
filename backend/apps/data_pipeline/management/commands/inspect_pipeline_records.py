...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "table_name",
            type=str,
            help="Table to inspect (e.g., students, academicclasses)"
        )
        parser.add_argument(
            "--sample-size",
            type=int,
            default=10,
            help="Number of random records to sample (default: 10)"
        )
        parser.add_argument(
            "--source-row",
            type=int,
            help="Inspect specific record by source row number"
        )
        parser.add_argument(
            "--stage-comparison",
            type=str,
            help="Compare specific stages (e.g., '3,4' or '1,3,5')"
        )
        parser.add_argument(
            "--export-csv",
            type=str,
        ...
        parser.add_argument(
            "--show-all-columns",
            action="store_true",
            help="Show all columns instead of key columns only"
        )
        parser.add_argument(
            "--business-rules-only",
            action="store_true",
            help="Focus on business rule validation fields"
        )
        parser.add_argument(
            "--include-invalid",
            action="store_true",
            help="Include invalid records from Stage 4"
        )
    ...
    def _get_stage_table_info(self, table_name: str) -> Dict[str, Dict]:
        ...
        stage_tables = {
            ...
            4: {
                "valid": f"{table_name}_stage4_valid",
                "invalid": f"{table_name}_stage4_invalid"
            },
            5: f"{table_name}_stage5_transformed",
            6: {
                "headers": f"{table_name}_headers",
                "lines": f"{table_name}_lines"
            }
        }
    ...
    def _show_stage_record(self, table_name: str, stage: int, info: Dict, source_row: int, options: Dict):
        ...
        stage_names = {
            1: "Raw Import", 2: "Data Profiling", 3: "Data Cleaning", 
            4: "Validation", 5: "Transformation", 6: "Record Splitting"
        }
    ...
    def _show_table_record(self, table_name: str, source_row: int, options: Dict, context: str):
        ...
        with connection.cursor() as cursor:
            ...
            columns_to_show = self._select_columns_to_show(
            ...
            for i, (col_name, value) in enumerate(zip(columns_to_show, record)):
    ...
    def _get_business_rule_columns(self, table_name: str, all_columns: List[str]) -> List[str]:
        ...
        if "students" in table_name:
            br_columns = [
                "name", "clean_name", "is_frozen", "is_sponsored", "sponsor_name", 
                "has_admin_fees", "birthdate", "birth_date_normalized",
                "emergency_contact_name", "_validation_errors", "_validation_score"
            ]
        elif "academicclasses" in table_name:
            br_columns = [
                "classid", "classname", "term", "_validation_errors", "_validation_score"
            ]
        elif "academiccoursetakers" in table_name:
            br_columns = [
                "studentid", "classid", "termid", "_validation_errors", "_validation_score"
            ]
        else:
    ...
    def _get_record_data(self, table_name: str, source_row: int) -> Optional[Dict]:
