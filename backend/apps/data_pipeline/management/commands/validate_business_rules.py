...
from typing import Any, Dict, List, Tuple, TypedDict, NotRequired, cast
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "table_name",
            type=str,
            help="Table to validate (e.g., students, academicclasses, or 'all')"
        )
        parser.add_argument(
            "--stage",
            type=int,
            choices=[3, 4, 5],
            default=4,
            help="Pipeline stage to validate (default: 4)"
        )
        parser.add_argument(
            "--detailed",
            action="store_true",
            help="Show detailed validation errors and examples"
        )
        parser.add_argument(
            "--export-failures",
            type=str,
            help="Export validation failures to CSV file"
        )
        parser.add_argument(
            "--threshold",
            type=float,
            default=95.0,
            help="Success rate threshold for warnings (default: 95.0%)"
        )
    ...
    def _validate_single_table(self, table_name: str, options: Dict, summary_only: bool = False) -> Dict[str, Any] | ValidationResults:
            ...
            for check_name, check_result in results["validation_checks"].items():
                ...
                if success_rate < threshold:
                    results["critical_failures"].append({
                        ...
                        "failed_count": check_result["failed"]
                    })
    ...
    def _validate_students_business_rules(self, cursor, stage_table: str, results: ValidationResults) -> None:
        ...
        if row:
            ...
            avg_consistency = (consistent_frozen + consistent_sponsored + consistent_admin_fees) / (3 * total) if total > 0 else 0
    ...
    def _validate_academiccoursetakers_business_rules(self, cursor, stage_table: str, results: ValidationResults) -> None:
    ...
    def _validate_financial_business_rules(self, cursor, stage_table: str, results: ValidationResults, table_name: str) -> None:
    ...
    def _show_detailed_results(self, table_name: str, results: ValidationResults, options: Dict) -> None:
        ...
        for check_name, check_result in results["validation_checks"].items():
            ...
            self.stdout.write(f"      Results: {check_result['passed']:,}/{check_result['total']:,} ({success_rate:.1f}%)")
        ...
        if results["critical_failures"]:
            ...
            for failure in results["critical_failures"]:
                self.stdout.write(f"   • {failure['check']}: {failure['success_rate']:.1f}% ({failure['failed_count']:,} failures)")
    ...
    def _show_overall_summary(self, results: List[Tuple[str, Dict[str, Any]]], options: Dict) -> None:
        ...
        passing_tables = sum(1 for _, result in results if result.get("overall_success_rate", 0) >= options["threshold"])
