...
class Command(BaseMigrationCommand):
    ...
    def _process_enrollment_record_from_db(
        ...
        try:
            ...
            # Convert zero-padded string student ID to integer
            try:
                student_id = int(record_id)  # Convert "00069" to 69
            except (ValueError, TypeError):
                self.record_rejection(
                    category="data_validation_error",
                    record_id=f"{record_id}-{class_id}",
                    reason=f"Invalid student ID format: {record_id}",
    ...
    def _get_or_create_class_part(
        ...
        class_part, created = ClassPart.objects.get_or_create(
            ...
            class_part_code=part_code,  # Use integer directly for integer field
            defaults={
