...
class LegacyStudent(BaseModel):
    """Pydantic model for legacy student data validation based on SQL Server DDL."""
    ...
    birthplace: str | None = Field(None, max_length=50)
    ...
    post_graduate_program_degree: str | None = Field(None, max_length=100)
    ...
    year_awarded: str | None = Field(None, max_length=10)
    other_credit_transfer_institution: str | None = Field(None, max_length=200)

    # Graduation info
    degree_awarded: str | None = Field(None, max_length=150)
    ...
    # Other
    transfer: str | None = Field(None, max_length=25)
...
class Command(BaseCommand):
    """Load legacy students data with validation."""

    help = "Load legacy students data from CSV with Pydantic validation"
    ...
    def _drop_table(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute("DROP TABLE IF EXISTS legacy_students")
            self.stdout.write("Dropped existing legacy_students table")

    def _create_table(self):
        """Create legacy_students table based on SQL Server DDL."""
        create_sql = """
        CREATE TABLE IF NOT EXISTS legacy_students (
            ...
            sel_program DECIMAL(10,2),
            selected_program VARCHAR(100),
            sel_major DECIMAL(10,2),
            selected_major VARCHAR(100),
            sel_faculty DECIMAL(10,2),
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write("Created legacy_students table")
