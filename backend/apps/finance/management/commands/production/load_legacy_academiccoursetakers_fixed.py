...
class LegacyAcademicCourseTaker(BaseModel):
    ...
    # Student and class info - FIXED LENGTHS
    id: str = Field(..., max_length=10, description="Student ID (5 digits, left padded)")
    classid: str = Field(..., max_length=255, description="Class ID (unique to class/time-of-day/section)")

    # Academic performance
    repeatnum: int | None = Field(None, description="Number of times repeated")
    lscore: float | None = Field(None, description="Lower score")
    uscore: float | None = Field(None, description="Upper score")
    credit: int | None = Field(None, description="Credit hours")
    gradepoint: float | None = Field(None)
    totalpoint: float | None = Field(None)
    ...
    # UI/Display fields - FIXED TO FLOAT
    color: float | None = Field(None)
    forecolor: float | None = Field(None)
    backcolor: float | None = Field(None)

    # Quick note and positioning
    quicknote: str | None = Field(None, max_length=50)
    pos: float | None = Field(None)
    gpos: float | None = Field(None)

    # Audit fields
    adder: float | None = Field(None)
    addtime: datetime | None = None
    lastupdate: datetime | None = None
    createddate: datetime | None = None
    modifieddate: datetime | None = None

    # Parsed fields from ClassID (computed in SQL Server, but we store them)
    section: str | None = Field(None, max_length=255)  # Computed field, using safe length
    timeslot: str | None = Field(None, max_length=255)  # Computed field, using safe length
    parsedtermid: str | None = Field(None, max_length=255)  # Computed field, using safe length
    parsedcoursecode: str | None = Field(None, max_length=255)  # Computed field, using safe length
    parsedlangcourse: str | None = Field(None, max_length=255)  # Computed field, using safe length

    # CRITICAL: Normalized fields for pricing and analysis - FIXED LENGTHS
    normalizedcourse: str | None = Field(None, max_length=15, description="Normalized course code")
    normalizedpart: str | None = Field(None, max_length=255, description="Normalized part - WAS MISSING!")
    normalizedsection: str | None = Field(None, max_length=15, description="CRITICAL for Reading Class pricing")
...
class Command(BaseCommand):
    """PRODUCTION LOADER - Load legacy academiccoursetakers data with EXACT DDL validation."""

    help = "PRODUCTION: Load legacy academiccoursetakers data from CSV with exact DDL field validation"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Loading academic course takers with exact SQL Server DDL specifications")
        ...
        # Write error log
        if error_count > 0:
            error_log_path = Path("academiccoursetakers_errors.log")
            with open(error_log_path, "w") as f:
                ...
                for error in error_details[:1000]:  # First 1000 errors
                    ...
                    f.write(f"  ID: {error['id']}, ClassID: {error['classid']}, IPK: {error['ipk']}\n\n")
    ...
    def _drop_table(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute("DROP TABLE IF EXISTS legacy_course_takers CASCADE")
            self.stdout.write("Dropped existing legacy_course_takers table")

    def _create_table(self):
        """Create legacy_course_takers table based on EXACT SQL Server DDL."""
        create_sql = """
        CREATE TABLE IF NOT EXISTS legacy_course_takers (
            ipk INTEGER PRIMARY KEY,
            id VARCHAR(10),                    -- FIXED: was 50, now 10
            classid VARCHAR(255),              -- FIXED: was 100, now 255
            repeatnum INTEGER,
            lscore FLOAT,                      -- Changed from DECIMAL to FLOAT
            uscore FLOAT,                      -- Changed from DECIMAL to FLOAT
            credit INTEGER,
            gradepoint FLOAT,                  -- Changed from DECIMAL to FLOAT
            totalpoint FLOAT,                  -- Changed from DECIMAL to FLOAT
            grade CHAR(10),                    -- FIXED: was VARCHAR, now CHAR
            previousgrade CHAR(10),            -- FIXED: was VARCHAR, now CHAR
            comment CHAR(100),                 -- FIXED: was VARCHAR, now CHAR
            passed INTEGER,
            remarks VARCHAR(50),
            color FLOAT,                       -- Changed from DECIMAL to FLOAT
            registermode VARCHAR(20),
            attendance CHAR(20),               -- FIXED: was VARCHAR, now CHAR
            forecolor FLOAT,                   -- FIXED: was INTEGER, now FLOAT
            backcolor FLOAT,                   -- FIXED: was INTEGER, now FLOAT
            quicknote CHAR(50),                -- FIXED: was VARCHAR, now CHAR
            pos FLOAT,                         -- Changed from DECIMAL to FLOAT
            gpos FLOAT,                        -- Changed from DECIMAL to FLOAT
            adder FLOAT,                       -- Changed from DECIMAL to FLOAT
            addtime TIMESTAMP,
            lastupdate TIMESTAMP,
            createddate TIMESTAMP,
            modifieddate TIMESTAMP,
            -- Computed fields (storing as VARCHAR with safe lengths)
            section VARCHAR(255),
            timeslot VARCHAR(255),
            parsedtermid VARCHAR(255),
            parsedcoursecode VARCHAR(255),
            parsedlangcourse VARCHAR(255),
            -- Normalized fields with EXACT DDL lengths
            normalizedcourse VARCHAR(15),      -- FIXED: was 50, now 15
            normalizedpart VARCHAR(255),       -- FIXED: was MISSING, now added
            normalizedsection VARCHAR(15),     -- FIXED: was 50, now 15
            normalizedtod VARCHAR(10),         -- FIXED: was 50, now 10
        ...
        CREATE INDEX idx_legacy_ct_ipk ON legacy_course_takers(ipk);
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write("Created legacy_course_takers table with EXACT DDL specifications")
