...
PRODUCTION LOADER - Validated data import for financial records
"""
...
class LegacyReceiptHeader(BaseModel):
    ...
    This represents payment/receipt records from the legacy system.
    """

    # Student and term info
    id: str | None = Field(None, max_length=50, description="Student ID (5 digits, left padded)")
    ...
    receipttype: str | None = Field(None, max_length=100, description="Type of receipt")
    notes: str | None = Field(None, max_length=500, description="Receipt notes - contains discount reasons")

    # System fields
    receiver: float | None = Field(None, description="Receiver/cashier ID")
    deleted: int | None = Field(None, description="Soft delete flag")
    name: str | None = Field(None, max_length=120, description="Student name")
    ...
    # Student info
    gender: str | None = Field(None, max_length=10, description="Student gender")
    curlevel: float | None = Field(None, description="Current level")

    # Transaction info
    cash_received: int | None = Field(None, description="Cash received flag")
    transtype: str | None = Field(None, max_length=100, description="Transaction type")
...
class Command(BaseCommand):
    """PRODUCTION LOADER - Load legacy receipt headers data with validation."""

    help = "PRODUCTION: Load legacy receipt headers (payment records) from CSV with Pydantic validation"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING("=== PRODUCTION DATA LOADER ==="))
        self.stdout.write("Loading financial receipt records - handle with care")
    ...
    def _create_table(self):
        """Create legacy_receipt_headers table based on SQL Server DDL."""
        create_sql = """
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write("Created legacy_receipt_headers table with financial analysis indexes")
